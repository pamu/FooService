...
trait Secured {
 /**
  ...
  /*
  def withSigningupUser(f: SigningupUser => Request[AnyContent] => Result) = isUserSignupAuthenticated{email => implicit request =>
    DAO.findSigningupUserWithEmail(email) match {
