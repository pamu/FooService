...
object DAO {
	...
	def saveUser(user: User): Long = db.withSession(implicit session => {
	...
	* */
	def getUserIdUsingEmail(email: String) = db.withTransaction(implicit tx => {
	  val userIdQ = for(user <- users.filter(_.email === email)) yield user.id
	  userIdQ.firstOption
	})
	/*
	def signinStatusEntryExistsFor(email: String) = db.withTransaction(implicit tx => {
	  val signinStatusQ = for((user, signinStatus) <- (users.filter(_.email === email).innerJoin(signinStatuses)).on(_.id === _.userId)) yield signinStatus
	  signinStatusQ.exists.run
	})
	* */
	...
	def signinOn(email: String) = db.withTransaction(implicit tx => {
	 val signinStatusQ = for((user, signinStatus) <- users.filter(_.email === email).innerJoin(signinStatuses).on(_.id === _.userId)) yield signinStatus.status
     signinStatusQ.update(1)
	})
	def signinOff(email: String) = db.withTransaction(implicit tx => {
	 val signinStatusQ = for((user, signinStatus) <- users.filter(_.email === email).innerJoin(signinStatuses).on(_.id === _.userId)) yield signinStatus.status
     signinStatusQ.update(0)
	})
	def saveSigninStatusEntry(userId: Long) = db.withTransaction(implicit tx => {
