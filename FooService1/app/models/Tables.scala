...
/*
case class RegPhone(simNumber: String, phoneNumber: String, id: Option[Long] = None)

class RegPhones(tag: Tag) extends Table[RegPhone](tag, "REG_PHONES") {
  ...
  def * = (simNumber, phoneNumber, id.?) <> (RegPhone.tupled, RegPhone.unapply)
}
...
class SigninStatuses(tag: Tag) extends Table[SigninStatus](tag, "SIGNIN_STATUSES"){
  ...
  def userIdFk = foreignKey("SIGNIN_STATUS_USER_ID_FK", userId, TableQuery[SigninStatuses])(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
}

case class SigningupUser(email: String, phno: String)

/*
class SigningupUsers(tag: Tag) extends Table[SigningupUser](tag, "SIGNINGUP_USERS"){
  def email = column[String]("EMAIL", O.NotNull)
  def phno = column[String]("PHNO", O.NotNull)
  def id = column[Long]("ID", O.PrimaryKey, O.NotNull, O.AutoInc)
  def * = (email, phno, id.?) <> (SigningupUser.tupled, SigningupUser.unapply)
}
*/
...
class SigningupUserEntries(tag: Tag) extends Table[SigningupUserEntry](tag, "SIGNINGUP_USER_ENTRIES"){
  //def suId = column[Long]("SIGNINGUP_USER_ID", O.NotNull)
  def email = column[String]("EMAIL", O.NotNull)
  def phno = column[String]("PHNO", O.NotNull)
  def timestamp = column[Timestamp]("TIMESTAMP", O.NotNull)
  ...
  //def suIdFK = foreignKey("SIGNINGUP_USER_ENTRY_SU_ID_FK", suId, TableQuery[SigningupUsers])(_.id, ForeignKeyAction.Cascade, ForeignKeyAction.Cascade)
}
